...
def loadOBJ(fliePath):
    ...
    for line in open(fliePath, "r"):
        ...
        if len(vals) == 0:
            continue
        if vals[0] == "v":
            v = vals[1:]## convert string to float and change type to list ##
            vertices.append(v)
            numVertices += 1
            if len(vals) == 7:
                vc = vals[1:]## convert string to float and change type to list ##
                vertexColors.append(vc)
        if vals[0] == "vt":
            vt = vals[1:]## convert string to float and change type to list ##
            uvs.append(vt)
            numUVs += 1
        if vals[0] == "vn":
            vn = vals[1:]## convert string to float and change type to list ##
            normals.append(vn)
            numNormals += 1
        if vals[0] == "f":
            fvID = []
