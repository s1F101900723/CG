...
def calcNewDragonCurvePoint(theta, p1, p2):
    ...
    print(before_p2)
    ##after_p2 = [np.dot(before_p2[0],rotation_matrix[0])*1/math.sqrt(2)+p1[0], np.dot(before_p2[1],rotation_matrix[1])*1/math.sqrt(2)+p1[1]]## Calculate new vertex after_p2 using rotation_matrix and before_p2 ##
    after_p2=np.dot(before_p2,rotation_matrix)*1/math.sqrt(2)
    print(after_p2)
    new_point = [after_p2[0]+p1[0], after_p2[1]+p1[1]]
    return new_point
...
def drawDragonCurve(generation, point1, point2):
    if generation == 0:
        ...
        newpoint = calcNewDragonCurvePoint(45, point1, point2)## Calculate new vertex using calcNewDragonCurvePoint as let point1 be a rotation center ##
        print (newpoint)
        glBegin(GL_LINES)
        glVertex2f(point1[0], point1[1])
        glVertex2f(newpoint[0], newpoint[1])
        glEnd()
        drawDragonCurve((generation-1), newpoint,point2)## This's a recursive program! ##
...
#    drawDragonCurve(2, [0.25, 0.5], [0.75, 0.5])   # for debug
