    ...
    for i in range(len(metabo)):
        ...
        for ix in range(minx, maxx):
            for iy in range(miny, maxy):
                ...
                if distance <= r:
                    ## calculate scalar_field by metaball function ##
...
def draw_2D_ScalarField(sf, min, max):
    ...
    for i in range(0, NX):
        ...
        for j in range(0, NY):
            ...
            ## calculate index for color_map based on value of sf[i][j] ##
            ## specify the color to glColor3f using color_map ##
            glVertex2f(x, y)

            index10 = int(255 * (sf[i+1][j] - min)/(max - min)) if sf[i+1][j] < max else 255
            ## specify the color to glColor3f using color_map ##
            ## specify 2D coordinate to glVertex2f ##

            ## calculate index (index11) for color_map based on value of sf[i+1][j+1] ##
            glColor3f(color_map[index11][0], color_map[index11][1], color_map[index11][2])
            ## specify 2D coordinate to glVertex2f ##

            ## calculate index for color_map based on value of sf[i][j+1] ##
            ## specify the color to glColor3f using color_map ##
            ## specify 2D coordinate to glVertex2f ##
    glEnd()
