...
def drawRationalBezier(cp, w, N, next_cp):
    ...
    p1 = np.zeros(3)
    p2 = ([cp[0][0], cp[0][1],cp[0][2]])
    p3 = np.zeros(3)
    p4 = ([next_cp[0][0], next_cp[0][1],next_cp[0][2]])
    leng = 0
    ...
    for t in np.linspace(0, 1, N):
        ...
        p1 = ([c[0]/sum_weight, c[1]/sum_weight,c[2]/sum_weight])
        p3 = ([next_c[0]/sum_weight, next_c[1]/sum_weight,next_c[2]/sum_weight])
        vec = np.array([p1[0]-p2[0],p1[1]-p2[1],p1[2]-p2[2]])
        leng += np.linalg.norm(vec)
        vec = np.array([p3[0]-p4[0],p3[1]-p4[1],p3[2]-p4[2]])
        leng += np.linalg.norm(vec)
        ...
        col = leng/Length(cp, w, N,next_cp)    
        p2 = ([cp[0][0], cp[0][1],cp[0][2]])
        p4 = ([next_cp[0][0], next_cp[0][1],next_cp[0][2]])
    glEnd()

def Length(cp, w, N, next_cp):
    number_cp = len(cp) # number of control points
    n = number_cp - 1   # value n of n-th Bezier
    p1 = np.zeros(3)
    p2 = ([cp[0][0], cp[0][1],cp[0][2]])
    ...
    for t in np.linspace(0, 1, N):
        ...
        vec = np.array([p1[0]-p2[0],p1[1]-p2[1],p1[2]-p2[2]])
        leng += np.linalg.norm(vec)
        p2=([c[0]/sum_weight, c[1]/sum_weight,c[2]/sum_weight])
        vec = np.array([p3[0]-p4[0],p3[1]-p4[1],p3[2]-p4[2]])
        leng += np.linalg.norm(vec)
        p4=([next_c[0]/sum_weight, next_c[1]/sum_weight,next_c[2]/sum_weight])## Specifying a vertex ##
    return leng
...
def display():
    ...
    line()
    # Draw Bezier curve
    cp1 = np.array([[-1.5, -0.5, 0.0], [x1-1.0, y1+1.0, 0.0], [x2+1.0, y2-2.5, 0.0], [x3+1.5, y3+0.5, 0.0]])
    cp2 = np.array([[-1.5, -0.5,-0.5], [x1-1.0, y1+1.0,-0.5], [x2+1.0, y2-2.5,-0.5], [x3+1.5, y3+0.5,-0.5]])
    cp3 = np.array([[-1.5, -0.5,-1.0], [x1-1.0, y1+1.0,-1.0], [x2+1.0, y2-2.5,-1.0], [x3+1.5, y3+0.5,-1.0]])
    cp4 = np.array([[-1.5, -0.5,-1.5], [x1-1.0, y1+1.0,-1.5], [x2+1.0, y2-2.5,-1.5], [x3+1.5, y3+0.5,-1.5]])
    w1 = np.array([1, 1, 1, 1])
